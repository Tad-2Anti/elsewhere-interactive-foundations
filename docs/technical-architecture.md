...
- `NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY` is environment-only and has no repository fallback. It must be `NEXT_PUBLIC_`-prefixed: Web3Forms rejects server-to-server submissions with `403` on free plans, so the browser calls `api.web3forms.com/submit` directly from `app/experience.tsx`, in parallel with `POST /api/requests` (database storage). Web3Forms's own key is designed to be safely exposed client-side. The two channels are independent: the form reports success when either the database write or the Web3Forms delivery succeeds. `POST /api/requests` alone returns `503` when the database is unconfigured and `502` when a configured database insert fails; Web3Forms failures are handled entirely client-side and don't affect the API route's response. Provider responses must report `success: true`; an HTTP success alone is insufficient.
